...
namespace Yahtzee_Simulation.Models;
...
public IEnumerable<YahzeeCup> GetAllCombinationsSorted()
{
    ...
    var possibleCombinations = new List<YahzeeCup>
    {
        ...
        new SmallStraight() { dice = this.dice },
        new FullHouse() { dice = this.dice },
        new FourOfAKind() { dice = this.dice },
        new ThreeOfAKind() { dice = this.dice },
        new Sixes() { dice = this.dice },
        new Fives() { dice = this.dice },
        new Fours() { dice = this.dice },
        new Threes() { dice = this.dice },
        new Twos() { dice = this.dice },
        new Ones() { dice = this.dice },
...
private int GetPriority(YahzeeCup combo)
{
    ...
    {
        Yahtzee => 6,
        LargeStraight or SmallStraight or FullHouse => 5,

        // Upper section prioriteras före Three/Four/Chance
        Ones or Twos or Threes or Fours or Fives or Sixes => 4,

        FourOfAKind or ThreeOfAKind => 3,
        Chance => 2,
        _ => 1
    };
}
